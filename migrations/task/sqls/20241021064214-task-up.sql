...
UPDATE "USER" SET role = 'COACH' WHERE 
    email = 'lee2000@hexschooltest.io' OR
    email = 'muscle@hexschooltest.io' OR
    email = 'starplatinum@hexschooltest.io';
-- 1-3 刪除：刪除USER 資料表中，用 Email 找到透明人，並刪除該筆資料
DELETE FROM "USER" WHERE email = 'opacity0@hexschooltest.io';
-- 1-4 查詢：取得USER 資料表目前所有用戶數量（提示：使用count函式）
SELECT count(*) as 所有用戶數量 FROM "USER" WHERE role = 'USER';
...
-- 2-2. 新增：在 `CREDIT_PURCHASE` 資料表，新增三筆資料：（請使用 name 欄位做子查詢）
    ...
    -- 2. `王小明` 購買 `21 堂組合包方案`
    -- 3. `好野人` 購買 `14 堂組合包方案`
INSERT INTO "CREDIT_PURCHASE" (credit_package_id,user_id,purchased_credits,price_paid)
VALUES 
(
...
SELECT "COACH".id, "SKILL".id FROM "COACH"
CROSS JOIN (SELECT id FROM "SKILL" WHERE name = '重訓') "SKILL";

INSERT INTO "COACH_LINK_SKILL" (coach_id,skill_id)
VALUES 
(
    (SELECT id FROM "COACH" where "COACH".user_id = 
